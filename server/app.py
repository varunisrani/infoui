...
def chat_assistant():
  ...
  <text x="540" y="800" font-family="Arial" font-size="60" text-anchor="middle" fill="#212529">Design Preview</text>
        ...
        if generate_svg and len(messages) > 0 and messages[-1]["role"] == "user":
            ...
            user_prompt = messages[-1]["content"]
            logger.info(f"Generating SVG from prompt: {user_prompt[:50]}...")
            
            # Use the existing SVG generation pipeline
            # Step 1: Pre-enhance the prompt
            pre_enhanced_prompt = pre_enhance_prompt(user_prompt)
            
            # Step 2: Further enhance the prompt
            enhanced_prompt = enhance_prompt_with_chat(pre_enhanced_prompt)
            
            # Step 3: Generate image
            gpt_image_base64, gpt_image_filename = generate_image_with_gpt(enhanced_prompt)
            
            # Step 4: Generate SVG
            svg_code = generate_svg_from_image(gpt_image_base64, enhanced_prompt)
            
            # Save the SVG
            svg_filename = save_svg(svg_code, prefix="svg")
            
            # Create AI response with SVG
            ai_response = f"I've created an SVG based on your request. Here it is:\n\n```svg\n{svg_code}\n```"
