...
export const AiAssistant = ({ editor, onClose }: AiAssistantProps) => {
                ...
                >
                  ...
                  {chat.svgCode && (
                    <div className="mt-2 overflow-hidden rounded-md bg-white h-[60px] w-[60px] border flex items-center justify-center">
                      <SvgRenderer
                        svgContent={chat.svgCode}
                        preserveAspectRatio="xMidYMid meet" 
                        className="scale-75"
                        onError={() => console.warn(`Error rendering SVG thumbnail for chat ${chat.id}`)}
                        fallback={
                          <div className="text-center p-1">
                            <span className="text-slate-500 text-[8px]">SVG Error</span>
                          </div>
                        }
                      />
                    </div>
      ...
      if (data.svg_path) {
        try {
          ...
          if (svgResponse.ok) {
            ...
            try {
              // Process the SVG through the normalizer to ensure it's valid and correctly formatted
              const { processed } = svgNormalizer.fullyProcessSvg(svgContent);
              setSvgCode(processed);
              toast.success("🎨 Design created successfully!");
            } catch (processingError) {
              console.error("Error processing SVG:", processingError);
              // Store original SVG if processing fails
              setSvgCode(svgContent);
              toast.success("🎨 Design created but may need adjustments");
            }
          } else {
  ...
  const useThisSvg = async () => {
    const currentSvgCode = svgCode || activeChat?.svgCode;
    if (!currentSvgCode) return;

    try {
      setIsAddingToCanvas(true);
      
      // Process the SVG to ensure it's properly formatted for the editor
      const { processed } = svgNormalizer.fullyProcessSvg(currentSvgCode);
  ...
  const saveToLibrary = async () => {
    ...
    try {
      ...
      const { processed, dataUrl } = svgNormalizer.fullyProcessSvg(currentSvgCode);
      
      // Save to storage with the processed SVG content
  ...
  // SVG Preview component that renders the SVG content using SvgRenderer
  const SvgPreview = () => {
    // Get SVG code from active chat or selected chat
    const currentChat = activeChat || chats.find((chat) => chat.id === selectedChatId);
    const currentSvgCode = svgCode || currentChat?.svgCode;
    
    // Handle case when no SVG is available
    if (!currentSvgCode) {
      return (
  ...
  // Add SVG to canvas (unused but kept for consistency)
  const addSvgToCanvas = () => {
    console.log("Add SVG to canvas called but not implemented in this component");
  };
